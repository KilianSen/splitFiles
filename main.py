...
def process_files(mode, input_path: str, output_path: str = None, max_size: int = None):
    ...
    if mode == 'archive':
        ...
        create_archive(input_path, output_path, max_size)
    elif mode == 'dearchive':
        dearchive(input_path)
    else:
...
def main():
    ...
    parser.add_argument('-i', '--input_path', type=str, required=True, help='Path to file to archive or directory to dearchive')
    parser.add_argument('-o', '--output_path', type=str, default=None, help='Path where to save the archive')
    parser.add_argument('-s', '--max_size', type=int, default=15, help='Max chunk size in whole megabytes')
